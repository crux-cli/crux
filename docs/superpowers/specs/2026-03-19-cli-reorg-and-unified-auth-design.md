...
exec python3 -m crux_cli.bridge \
  ...
  --header "Authorization: Bearer \$CRUX_AUTH_TOKEN"
```
...
- Forwards each as an HTTP POST to `--url` with `--header` values
...
- Supports `--header` flag (repeatable) for injecting arbitrary headers
- Token value comes from environment variable (set by launcher), never from arguments
