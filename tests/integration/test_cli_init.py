...
class TestCruxInit:
    def test_creates_project_directory(self, crux_env):
        env, root = crux_env
        run_crux("init", "myproject", env=env, cwd=str(root))
    ...
    def test_exits_nonzero_if_project_exists(self, crux_env):
        ...
        result = run_crux("init", "existing", env=env, cwd=str(root))
        assert result.returncode != 0
        assert "already exists" in result.stdout

    def test_success_message_shown(self, crux_env):
        env, root = crux_env
        result = run_crux("init", "myproject", env=env, cwd=str(root))
        assert result.returncode == 0
        assert "myproject" in result.stdout

    def test_init_current_dir(self, crux_env):
        env, root = crux_env
        project = root / "my-app"
        project.mkdir()
        result = run_crux("init", env=env, cwd=str(project))
        assert result.returncode == 0
        assert (project / "crux.json").exists()

    def test_init_registers_project(self, crux_env):
        env, root = crux_env
        run_crux("init", "tracked", env=env, cwd=str(root))
